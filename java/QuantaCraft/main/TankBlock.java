...
public class TankBlock extends BlockContainer{
	...
	private boolean opa;
	int TexNum;
	public IIcon[] textures = new IIcon[19];
	...
	String TexName;
	public TankBlock( String blockName, Material blockMaterial, float hardness,boolean opaque,int lo,String textureName){
    ...
    public void registerBlockIcons(IIconRegister icon) {
        ...
        textures[17] = icon.registerIcon(modMain.modid.toLowerCase() + ":" + TexName + "_top");
        textures[18] = icon.registerIcon(modMain.modid.toLowerCase() + ":" + TexName + "_bottom");
      }
    ...
    public IIcon getIcon(IBlockAccess world, int blockX, int blockY, int blockZ, int side)
    {
    	...
    	if (side==1)  TexNum = 18;
    	if (side==0)  TexNum = 17;
    	if (side==1 && world.getBlock(blockX,blockY-1,blockZ) == this){
    		TexNum = 0;
    	}
    	if (side==0 && world.getBlock(blockX,blockY+1,blockZ) == this){
    		TexNum = 0;
    	}
