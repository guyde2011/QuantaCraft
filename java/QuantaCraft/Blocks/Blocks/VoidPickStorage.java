...
public class VoidPickStorage extends BlockContainer{
	public TileEntityVoidPickStorage hi;
	public ItemStack placedBy;
    public VoidPickStorage(int par1, String name, Material blockMaterial,float hardness) {
    	super(blockMaterial);
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int i, float a, float b, float c)
    {
    	if (player.getHeldItem()!=null && player.getHeldItem().getItem()==modMain.voidGem){
    		hi = (TileEntityVoidPickStorage) world.getTileEntity(x, y, z);
    		hi.storageMax +=5000;
    		player.setCurrentItemOrArmor(0, null);
    	}
    	if (!world.isRemote) {
    		hi = (TileEntityVoidPickStorage) world.getTileEntity(x, y, z);
    		player.openGui(modMain.instance, 6, world, x, y, z);
    		return true;
    	} else {
    		return false;
    	}
    ...
    private void saveItems(World world, int x, int y, int z , Block block){
            ...
            TileEntityVoidPickStorage store =( TileEntityVoidPickStorage) tileEntity;


            ItemStack item = new ItemStack(modMain.pickVoidStorage);
            ...
            Items.setInteger("MaxStorage",store.storageMax);
            Items.setInteger("StorageAmount",store.storage);
            NBTTagCompound hi = new NBTTagCompound();
            if (store.storedItem!=null){
            	store.storedItem.writeToNBT(hi);
            	Items.setTag("StorageItem", hi);
            }
    ...
    public TileEntity createNewTileEntity(World world, int var2) {
    	this.hi=new TileEntityVoidPickStorage();    
